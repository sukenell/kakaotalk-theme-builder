...
test("bubble uploads expose nine-patch detail controls that drive preview and export rendering", async () => {
  ...
  assert.match(html, /class="bubble-detail-action-icon reset-icon"/);
  assert.doesNotMatch(html, />안녕하세요<\/p>/);
  assert.doesNotMatch(html, /data-nine-patch-reset>↺<\/button>/);
  assert.match(css, /\.bubble-detail-preview\s*\{/);
  assert.match(css, /\.bubble-detail-header \.bubble-detail-actions\s*\{[\s\S]*display: inline-flex;[\s\S]*flex-wrap: nowrap;/);
  assert.match(css, /\.bubble-detail-action-icon\s*\{[\s\S]*width: 18px;[\s\S]*height: 18px;/);
  ...
  assert.match(css, /\.nine-patch-preview::before\s*\{/);
  assert.match(css, /width: var\(--nine-source-width, 100%\);/);
  assert.match(css, /height: var\(--nine-source-height, 100%\);/);
  assert.match(app, /function renderBubbleDetailControls/);
  assert.match(app, /control\.dataset\.ninePatchControl = field;/);
  assert.match(app, /field === "paddingX"/);
  assert.match(app, /field === "paddingY"/);
